                     ...
                     int num_local, int local_rank, int local_procs_0, int rank)
{
	...
	pmi_errno = PMI_Barrier();
        ...
        if (local_rank == 0){
            ...
            gettimeofday(&tv_stop, NULL);
            printf("shm_seg_commit-PMI_KVS_Put %8.8f\n", (tv_stop.tv_sec - tv_start.tv_sec) + (tv_stop.tv_usec - tv_start.tv_usec)/1000000.0);
            gettimeofday(&tv_start, NULL);
            pmi_errno = PMI_Barrier();
            MPIR_ERR_CHKANDJUMP1 (pmi_errno != PMI_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmi_barrier", "**pmi_barrier %d", pmi_errno);
            gettimeofday(&tv_stop, NULL);
            printf("shm_seg_commit-PMI_Barrier_after_Put %8.8f\n", (tv_stop.tv_sec - tv_start.tv_sec) + (tv_stop.tv_usec - tv_start.tv_usec)/1000000.0);
            gettimeofday(&tv_start, NULL);
        }
        else
        {
            ...
            MPIR_ERR_CHKANDJUMP1 (pmi_errno != PMI_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmi_barrier", "**pmi_barrier %d", pmi_errno);
            gettimeofday(&tv_stop, NULL);
            printf("shm_seg_commit-PMI_Barrier_befor_Get %8.8f\n", (tv_stop.tv_sec - tv_start.tv_sec) + (tv_stop.tv_usec - tv_start.tv_usec)/1000000.0);
            gettimeofday(&tv_start, NULL);
            ...
            if (mpi_errno) MPIR_ERR_POP(mpi_errno);
            gettimeofday(&tv_stop, NULL);
            printf("shm_seg_commit-PMI_KVS_Get %8.8f\n", (tv_stop.tv_sec - tv_start.tv_sec) + (tv_stop.tv_usec - tv_start.tv_usec)/1000000.0);
            gettimeofday(&tv_start, NULL);
        }
 ...
 fn_exit:
    ...
    gettimeofday(&tv_stop, NULL);
    printf("shm_seg_commit-Epilogue %8.8f\n", (tv_stop.tv_sec - tv_start.tv_sec) + (tv_stop.tv_usec - tv_start.tv_usec)/1000000.0);
    gettimeofday(&tv_start, NULL);
    return mpi_errno;
 fn_fail:
